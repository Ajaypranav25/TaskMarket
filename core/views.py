    ...
    if request.method == 'POST':
        ...
        if errors:
            return render(request, 'core/register.html', {
                'errors': errors, 'levels': Level.objects.all()
            })
        ...
        login(request, user)
        messages.success(request, f'Welcome to TaskMarket, {username}!')
        return redirect('dashboard')

    return render(request, 'core/register.html', {'levels': Level.objects.all()})
...
def task_list(request):
    ...
    # Mark accessibility and submission status
    task_data = []
    for task in tasks:
        task_data.append({
            'task': task,
...
def submission_detail(request, pk):
    ...
    # Only the worker or company can see
    ...
    return render(request, 'core/submission_detail.html', ctx)
...
def task_submissions(request, pk):
    """Company view of all submissions for a task."""
    task = get_object_or_404(Task, pk=pk, company=request.user)
